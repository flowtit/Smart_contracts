...
contract BirthdayPayout {
  ...
  function findBirthdays() view public onlyOwner returns(uint256[] memory) {
    require(getTeammatesNumber() > 0, "No teammates in the database");
    uint256[] memory result = new uint256[](getTeammatesNumber());
    uint256 count = 0;
  ...
  function BBDay() public onlyOwner{
    require(getTeammatesNumber()>0,"No teammates in the database");
    for(uint256 i=0;i<getTeammatesNumber();i++){
